...
def get_prediction(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Import the implementation from the GetPrediction directory
        # This avoids duplicating all the complex code here
        import sys
        from pathlib import Path
        
        # Ensure the GetPrediction directory is in the path
        prediction_path = Path(__file__).parent / 'GetPrediction'
        if str(prediction_path) not in sys.path:
            sys.path.append(str(prediction_path))
        
        # Import the necessary functions from the GetPrediction implementation
        from GetPrediction.function_app import load_image, prepare_inference_data, encode_image_to_png_bytes, is_image_in_range
        from GetPrediction.function_app import load_model_path
